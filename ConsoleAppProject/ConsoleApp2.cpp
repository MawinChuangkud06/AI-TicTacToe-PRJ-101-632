...
// This is Unfixed to Compare with Fixed One
...
// Below is Fixed One
// This is so Painful There Also Issue with this code
#ifndef TICTACTOE_H
#define TICTACTOE_H
...
// Below is Fixed One By BlackBox And Me :P
#ifndef TICTACTOE_H
#define TICTACTOE_H
...
public:
    ...
    TicTacToe()
    {
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                this->Board[i][j] = '1' + (i * 3 + j);
            }
        }        
        std::srand(time(0));
        StartTurn = std::rand() % 2;
    }
    ...
    void StartAIGame()
    {
        this->ChooseTeam();
        this->ChooseDiff();
        this->Turn = (this->StartTurn == 0) ? this->Player : this->AI;
        while (true)
        {
            this->PrintBoard();
            if (this->IsGameOver())
...
void AIFill()
{
    switch (this->Diff)
    {
        ...
        break;
        case 'h': // Hard (using Minimax algorithm)
        {
            // to block player
            std::pair<int, int> avaPlayer = this->FindAvaSpot(this->Player);
            if (this->IsAbleToFill(avaPlayer.first, avaPlayer.second))
            {
                this->Board[avaPlayer.first][avaPlayer.second] = this->AI;
                return;
            }
            long bestScore = std::numeric_limits<long>::min();
            std::pair<int, int> bestMove = std::make_pair(-1, -1); // Initialize best move
            ...
            for (int i = 0; i < 3; i++)
            {
                for (int j = 0; j < 3; j++)
                {
                    if (this->IsAbleToFill(i, j))
                    {
                        char temp = this->Board[i][j];
                        this->Board[i][j] = this->AI;
                        long score = this->MiniMax(false, -1000000, 1000000, 0);
                        this->Board[i][j] = temp;
                        if (score > bestScore)
                        {
                            bestScore = score;
        ...
        break;

            {
                ...
                if (this->IsAbleToFill(1, 1))
                {
                    this->Board[1][1] = this->AI;
                    return;
                }
    ...
    long MiniMax(bool isMaximizingPlayer, long alpha, long beta, unsigned long depth)
    {
        ...
        if (isMaximizingPlayer)
        {
            long maxEval = std::numeric_limits<long>::min();
            for (int i = 0; i < 3; i++)
            {
                for (int j = 0; j < 3; j++)
                {
        ...
        {
            long minEval = std::numeric_limits<long>::max();
            for (int i = 0; i < 3; i++)
            {
                for (int j = 0; j < 3; j++)
                {
