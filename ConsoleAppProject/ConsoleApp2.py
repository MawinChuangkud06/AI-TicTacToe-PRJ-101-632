...
#This Code Below Is The Code Below Is AI GameMode Only Code So Try it if You Like
"""
...
if __name__ == "__main__": 
    ...
    ttt1.StartAIGame()
"""
# This Code Below Has Additonal Game Mode Which are Now 2
# 1 Play Alone, 2 Play With AI
from math import inf
from random import randint
from pyfiglet import figlet_format
class TicTacToe:
    ...
    def PlayerFill(self, row, col) -> None:
        #safety
    ...
    def AutoPlayRoundForAI(self):
        if self.Turn == self.Player:
            ...
            self.Turn = self.Player
    # Game 
    def AutoPlayRoundForPlayer(self):
        while True:
            try:
                plrCin = int(input("Current Player : {}\ninput From 1-9 to Fill in The Board : ".format(self.Player)))
                if plrCin < 1 or plrCin > 9:
                    print("invaild input please TryAgain")
                    continue
                row, col = (plrCin - 1) // 3, (plrCin - 1) % 3
                if not self.IsAbleToFill(row, col):
                    print("Board already Filled Choose Another spot")
    ...
    def StartAloneGame(self):
        ...
        while True:
            self.DrawBoard()
            if self.IsGameOver():
                print("Game Over Winner : {}".format(self.PrevPlayed))
                break
            if self.IsTie():
                print("Game Tie!")
                break
            self.AutoPlayRoundForPlayer()

    def ChooseGameModeAndPlay(self):
        while True:
            n = input("Choose You GameMode\n1: Play Alone\n2: Play With AI\ninput Here To Choose : ")
            match n:
                case '1':
                    self.StartAloneGame()
                case '2':
                    self.StartAIGame()
                case _:
                    print("invaild GameMode Choose Again")
                    continue
            break
    def StartAIGame(self):
        self.ChooseTeam()
        ...
        while True:
            ...
            if self.IsTie():
                print("Game Tie!")
                break
            self.AutoPlayRoundForAI()
                
# Test Result
if __name__ == "__main__": 
    print(figlet_format("TicTacToe..."))
    ttt1 = TicTacToe()
    ttt1.ChooseGameModeAndPlay()
