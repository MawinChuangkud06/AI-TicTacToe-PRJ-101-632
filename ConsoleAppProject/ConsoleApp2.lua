...
function tictactoe:AutoPlayRoundForAI()
...
function tictactoe:AutoPlayRoundForPlayer()
    local row, col
    ::again::
    io.write(string.format("Player Turn! Currently Player Is %s\nChoose To Fill From 1-9\ninput Here : ", self.Player))
    local plrCin = tonumber(io.read())
    if (plrCin < 1 or plrCin > 9) then
        print("Invalid input. Please enter a number between 1 and 9.")
        return
    end
    row = math.ceil(plrCin / 3)
    col = (plrCin - 1) % 3 + 1
    if self:IsAbleToFill(row, col) then
        self:PlayerFill(row, col)
        self.PrevPlayed = self.Player
...
function tictactoe:ChooseGameModeAndPlay()
    local n
    ::again::
    io.write("Choose Game Mode\n1: Play Alone\n2: Play With AI\nInput Here : ")
    n = string.lower(io.read())
    if n == "1" then
        self:StartAloneGame()
    elseif n == "2" then
        self:StartAIGame()
...
function tictactoe:StartAloneGame()
    self:ChooseTeam()
    self:StartTurn()
    while true do
        --os.execute("clear")
